...
export async function loader() {
  const becomeAsponser = await client.getBecomeAsponser();
  const page = await client.getPage("becomeAsponser");
...
export default function becomeAsponser() {
  const { becomeAsponser } = useLoaderData();
  return (
    <div className="px-4 sm:px-48">
      <Title title="Become a Sponser" />
      <div className="grid gap-4 sm:grid-cols-3">
        {becomeAsponser.map((project) => {
          return (
            <motion.div
              key={becomeAsponser.title}
